...
export class AuthService {
    ...
    */
    ...
    public registerDriver() {
        this.register(this.registerUrl + 'driver', 'ROLE_DRIVER', '/driver/activation')
    }

    public registerPassenger() {
        this.register(this.registerUrl + 'passenger', 'ROLE_PASSENGER', '/passenger/activation')
    }
            ...
            .pipe(
                map(resp => {
                    if (this.user.roleUsers[0].role.authority === 'ADMIN') {
                        ...
                        else if (this.user.roleUsers[0].role.authority === 'DRIVER') {
                        this.router.navigate(['/driver/login']);
                        }
                        else if (this.user.roleUsers[0].role.authority === 'PASSENGER') {
