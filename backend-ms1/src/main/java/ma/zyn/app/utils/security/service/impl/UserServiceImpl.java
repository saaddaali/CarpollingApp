...
import ma.zyn.app.utils.security.bean.ModelPermissionUser;
...
import ma.zyn.app.utils.security.dao.facade.core.UserDao;
import ma.zyn.app.utils.security.dao.specification.core.UserSpecification;
import ma.zyn.app.utils.security.service.facade.*;
import ma.zyn.app.utils.service.AbstractServiceImpl;
import ma.zyn.app.utils.util.ListUtil;
...
public class UserServiceImpl extends AbstractServiceImpl<User, UserCriteria, UserDao> implements UserService {
    ...
    private User createWithEnable(User t, boolean enable) {
        ...
        if (foundedUserByUsername != null || foundedUserByEmail != null) return null;
			...
			else if(roleAsString.equals(AuthoritiesConstants.PASSENGER)){
				saved = passengerService.create(convertToPassenger(t));
			}
 
 
 
 
			else if(roleAsString.equals(AuthoritiesConstants.DRIVER)){
				saved = driverService.create(convertToDriver(t));
			}
	 ...
	 private Passenger convertToPassenger(User user) {
    ...
    @Autowired
    private PassengerAdminService passengerService;
    @Autowired
